defmodule MixDeployTest do
  ...
  describe "expand_vars/2" do
    ...
    test "list of terms expands vars" do
      assert Mix.Tasks.Deploy.expand_vars([:deploy_dir, "/etc"], deploy_dir: "/srv/foo") ==
               "/srv/foo/etc"

      assert Mix.Tasks.Deploy.expand_vars(["!", :deploy_dir, "/bin/sync"], deploy_dir: "/srv/foo") ==
               "!/srv/foo/bin/sync"
    end

    test "handles env vars" do
      assert Mix.Tasks.Deploy.expand_vars(["RELEASE_MUTABLE_DIR=", :runtime_dir],
               runtime_dir: "/run/foo"
